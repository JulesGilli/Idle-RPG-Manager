-- 0048_daily_reward_and_team_power.sql
-- (1) Récompense journalière : table de suivi des réclamations (une par jour).
-- (2) Leaderboard : la puissance = somme des 5 MEILLEURS héros (puissance d'équipe)
--     au lieu de la somme de tous les héros possédés.

-- ------------------------------------------------------------- daily_claims
create table public.daily_claims (
  player_id       uuid primary key references public.profiles (id) on delete cascade,
  last_claim_date date,
  day_index       int not null default 0, -- dernier jour réclamé dans le cycle (0..10)
  updated_at      timestamptz not null default now()
);

alter table public.daily_claims enable row level security;

-- Lecture de sa propre ligne ; écriture réservée au service_role (daily-reward).
create policy "daily_claims own" on public.daily_claims
  for select to authenticated using (player_id = auth.uid());

-- ----------------------------------------------------- leaderboard (team power)
