...
const GUERRIER: SkillBranch[] = [
  ...
  { id: 2, name: 'Berserker', color: '#dc2626', nodes: [
    passive('g_ber_rage', 2, 'Rage montante', '💢', 'Plus tes PV sont bas, plus tu tapes fort (+dégâts sous 50% PV).',
      { passives: [{ type: 'rage', value: 0.03, valuePerRank: 0.02 }] }),
    passive('g_ber_oeil', 2, 'Œil du tueur', '🎯', '+chance de coup critique (×2 dégâts).',
      { passives: [{ type: 'crit', value: 0.04, valuePerRank: 0.04 }] }),
    ...
    active('g_ber_brutale', 2, 'Frappe brutale', '🪓', 'Ignore une partie de l’armure de la cible.',
      { abilities: [{ kind: 'armor_pen', value: 0.15, valuePerRank: 0.15 }] }),
  ...
  { id: 3, name: 'Rempart', color: '#3b82f6', nodes: [
    passive('g_rem_parade', 3, 'Parade', '🛡️', 'Regagne chaque tour une barrière absorbant un % de tes PV max.',
      { abilities: [{ kind: 'barrier', value: 0.05, valuePerRank: 0.03 }] }),
...
const ARCHER: SkillBranch[] = [
  { id: 1, name: 'Vipère', color: '#22c55e', nodes: [
    ...
    active('a_vip_volee', 1, 'Volée toxique', '🏹', 'Périodiquement, empoisonne tous les ennemis d’un coup.',
      { abilities: [{ kind: 'autocast', everyRounds: 6, everyRoundsPerRank: -1,
        action: { type: 'aoe', dmgMult: 0.5, status: 'poison', statusChance: 1, statusPotency: 0.15, statusDuration: 3 } }] }),
    ultimate('a_vip_fleau', 1, 'Fléau viral', '💀', 'Dégâts fortement amplifiés contre les cibles empoisonnées.',
      { abilities: [{ kind: 'amp_vs_status', status: 'poison', bonus: 0, bonusPerRank: 0.5 }] }),
  ] },
  { id: 2, name: 'Tempête', color: '#06b6d4', nodes: [
    ...
    active('a_tem_pluie', 2, 'Pluie de flèches', '🏹', 'Périodiquement, tire sur tous les ennemis.',
      { abilities: [{ kind: 'autocast', everyRounds: 5, everyRoundsPerRank: -1, action: { type: 'aoe', dmgMult: 1 } }] }),
    ultimate('a_tem_ouragan', 2, 'Ouragan', '🌀', 'Périodiquement, frappe TOUS les ennemis 2 fois d’affilée.',
      { abilities: [{ kind: 'autocast', everyRounds: 8, everyRoundsPerRank: -2,
        action: { type: 'multi_hit', hits: 2, dmgMult: 0.6 } }] }),
  ] },
  { id: 3, name: 'Œil de faucon', color: '#f59e0b', nodes: [
    passive('a_oeil_visee', 3, 'Visée mortelle', '🎯', '+chance de coup critique.',
      { passives: [{ type: 'crit', value: 0.05, valuePerRank: 0.05 }] }),
    ...
    passive('a_oeil_grace', 3, 'Coup de grâce', '🏹', 'Dégâts bonus massifs contre les cibles à bas PV.',
      { passives: [{ type: 'execute', value: 0.3, valuePerRank: 0.1 }] }),
    active('a_oeil_perforante', 3, 'Flèche perforante', '🪶', 'Périodiquement, étourdit un ennemi pendant 2 tours.',
      { abilities: [{ kind: 'autocast', everyRounds: 6, everyRoundsPerRank: -1,
        action: { type: 'nuke', dmgMult: 0.8, status: 'stun', statusDuration: 2 } }] }),
...
const PALADIN: SkillBranch[] = [
  { id: 1, name: 'Bastion', color: '#cbd5e1', nodes: [
    ...
    ultimate('p_bas_rempart', 1, 'Rempart inébranlable', '🏔️', 'Forte réduction des dégâts subis pendant que tu forces l’agro.',
      { passives: [{ type: 'shield', value: 0.1, valuePerRank: 0.1 }], abilities: [{ kind: 'taunt', everyRounds: 5, duration: 3 }] }),
  ] },
