...
describe('craftItem', () => {
  it('produit un objet spécifique nommé "modèle + composant"', () => {
    ...
    const item = craftItem(base, mat, createRng(42));
  ...
  it('le tier de l’objet suit le tier de craft du composant', () => {
    ...
    for (const mat of FORGE_MATERIALS) {
      const item = craftItem(base, mat, createRng(7));
  ...
  it('déterministe pour une même seed', () => {
    ...
    const a = craftItem(base, mat, createRng(123));
    const b = craftItem(base, mat, createRng(123));
    expect(a).toEqual(b);
  });

  it('le thème du composant oriente les stats (givre → DEF sur une arme)', () => {
    const base = getBase('epee')!;
    const givre = getMaterialTier('givre')!;
    const item = craftItem(base, givre, createRng(11));
    expect(item.def_bonus).toBeGreaterThan(0); // une épée "neutre" n'a pas de DEF
    const chene = getMaterialTier('chene')!;
    const neutral = craftItem(base, chene, createRng(11));
    expect(neutral.def_bonus).toBe(0);
  });
  ...
  it('un composant de zone supérieure donne des objets plus puissants (en moyenne)', () => {
    ...
    let sum10 = 0;
    for (let s = 0; s < 200; s++) {
      sum1 += craftItem(base, z1, createRng(s)).atk_bonus;
      sum10 += craftItem(base, z10, createRng(s)).atk_bonus;
    }
  ...
  it('les stats craftées restent dans la range affichée', () => {
    for (const base of FORGE_BASES) {
      for (const mat of FORGE_MATERIALS) {
        ...
        for (let s = 0; s < 50; s++) {
          const item = craftItem(base, mat, createRng(s * 31 + 1));
  ...
  it('la distribution de rareté suit les % globaux (grossièrement)', () => {
    ...
    const N = 5000;
    for (let s = 0; s < N; s++) {
      const r = craftItem(base, mat, createRng(s)).rarity;
  ...
  it('toutes les bases sont craftables avec tous les composants', () => {
    for (const base of FORGE_BASES) {
      for (const mat of FORGE_MATERIALS) {
        const item = craftItem(base, mat, createRng(1));
