    ...
    <div className="anim-fade fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
      <div className="panel anim-pop max-h-[90vh] w-full max-w-2xl overflow-y-auto p-5">
        ...
        <p className="mb-4 text-xs text-[var(--color-muted)]">
          Connecte-toi chaque jour pour récolter des <strong>matériaux</strong>. Le{' '}
          <strong>jour 10</strong> t'offre un <strong>objet ultime de zone 10</strong>. Rater un jour
          ...
          {DAILY_REWARDS.map((r) => {
            ...
            return (
              ...
              >
                ...
                {isTen && (
                  <span className="rounded bg-[var(--color-gold)]/20 px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-wide text-[var(--color-gold-soft)]">
                    Objet ultime Z10
                  </span>
                )}
        ...
        {claim.data?.ok && (
          <p className="mb-2 text-sm text-[var(--color-gold-soft)]">
            ...
            {claim.data.item ? `Objet ultime obtenu : ${claim.data.item.name}.` : 'Matériaux crédités.'}
          </p>
        )}
